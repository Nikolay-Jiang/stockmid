...
import { RSI_THRESHOLDS, PRICE_THRESHOLDS, BOLLINGER_THRESHOLDS, TIME_WINDOWS, W_DETECT_LOOSE, PORTFOLIO_DEFAULTS } from '@shared/constants/trading-constants';
...
// ============================================================
// Types/Enums/Classes extracted from simtrade-router.ts (T13)
// ============================================================
...
function runHQ(dayrpts: t_StockDayReport[], index: number, myoper: stockOP): void {
    const element = dayrpts[index];
...
function getDataAnalysis(dayrps: t_StockDayReport[]): void {
...
function findmoneyAnalysis(dayrpts: t_StockDayReport[]): void {
...
// ============================================================
// Business logic wrapper methods (extracted from router handlers)
// ============================================================

interface EvalResultBase {
    enddate: Date;
    tempCount: number;
    filterCount: number;
    iCountGood: number;
...
async function simulateFromPredictions(startday: string): Promise<{ total: number; iCountLow: number; iCountAfterNoon: number }> {
...
async function runStatistics(startday: string, endday: string, stockcode: string): Promise<{ temp: number; temprate: string; tempStat: string } | null> {
...
async function evaluateFindW(endday: string): Promise<EvalResultBase | null> {
    var enddate: Date = new Date(endday);
    enddate.setHours(8, 0, 0, 0);
...
async function evaluateFindYZM(endday: string): Promise<(EvalResultBase & { dayrptsRule: wresult[] }) | null> {
    var enddate: Date = new Date(endday);
    enddate.setHours(8, 0, 0, 0);
...
async function findAndSaveYZMOnline(): Promise<{ count: number; message: string } | null> {
...
async function findAndSaveYZMByDay(endday: string): Promise<{ count: number } | null> {
    var enddate: Date = new Date(endday);
    enddate.setHours(8, 0, 0, 0);
    if (enddate.getDay() == 0 || enddate.getDay() == 6) { return null; }
...
async function findAndSaveWOnline(): Promise<{ enddate: Date; txtresult: string } | null> {
    var enddate: Date = new Date();
    enddate.setHours(8, 0, 0, 0);
